...
static double get_runtime()
{
    ...
    if (stop == (std::clock_t)-1 || start_time == (std::clock_t)-1)
        return 0.0;
    return static_cast<double>(stop - start_time) / static_cast<double>(CLOCKS_PER_SEC);
#else
    struct tms stop;
    times(&stop);
...
public:
    ...
    void flip_FPS(int flipvar, long long &time_steps);
    void update_goodvarstack1(int flipvar, long long &time_steps);
    void update_goodvarstack2(int flipvar);
    void update_goodvarstack_FPS(int flipvar, long long &time_steps);
    int pick_var(long long &time_steps);
    int pick_var_FPS(long long &time_steps);
    int pick_var_FPS2(long long &time_steps);
...
inline void NUWEIGHTING::settings()
{
    ...
    if (1 == problem_weighted)
    {
        ...
        if (num_hclauses > 0) // weighted partial
        {
            ...
            if (num_sclauses == 0)
            {
                coe_tuned_weight = 1.0 / (1 + double(top_clause_weight - 1) / (double)(num_sclauses + 1));
            }
            else
    ...
    else
    {
        ...
        if (num_hclauses > 0) // unweighted partial
        {
            ...
            soft_increase_ratio = 1.00003; // 1.00072;
        }
...
inline void NUWEIGHTING::soft_increase_weights_not_partial(long long &time_steps)
{
    int i, c, v;
...
inline int NUWEIGHTING::pick_var_FPS(long long &time_steps)
{
    ...
    double max_score1 = (double)LONG_LONG_MIN, max_score2 = (double)LONG_LONG_MIN;
    int num1, num2;
    ...
    for (i = 0; i < selected_nums; i++)
    {
        flip(best_vars[i], time_steps);
        if (goodvar_stack_fill_pointer > 0)
        {
            if (goodvar_stack_fill_pointer < sv_num)
            {
                time_steps++;
    ...
    {
        time_steps += 2;
        flip(best_vars[num2], time_steps);
        time_stamp[best_vars[num2]] = step2;
        return vars2[num2];
    }
}
...
inline void NUWEIGHTING::update_goodvarstack1(int flipvar, long long &time_steps)
{
    int v;
...
inline void NUWEIGHTING::update_goodvarstack_FPS(int flipvar, long long &time_steps)
{
    int v;
...
inline int NUWEIGHTING::pick_var_FPS2(long long &time_steps)
{
    if (hardunsat_stack_fill_pointer + softunsat_stack_fill_pointer == 0)
    {
        return -1;
    ...
    for (i = 0; i < selected_nums; i++)
    {
        flip_FPS(best_vars[i], time_steps);
    ...
    {
        time_steps += 2;
        flip2(best_vars[num2], time_steps); // flip(vars2[num2]);
        time_stamp[best_vars[num2]] = step2;
        // time_stamp[vars2[num2]] = step;
        return vars2[num2];
    }
}
inline void NUWEIGHTING::flip_FPS(int flipvar, long long &time_steps)
{
    int i, v, c;
    int index;
    clauselit *clause_c;
    ...
    update_goodvarstack_FPS(flipvar, time_steps);
}
