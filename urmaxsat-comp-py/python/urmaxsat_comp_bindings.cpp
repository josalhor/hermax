...
public:
    ...
    UWrMaxSATComp() : solver(ipamir_init()), num_vars(0) {
        if (!solver) {
            throw std::runtime_error("Failed to initialize UWrMaxSAT 1.4 solver.");
        }
    }
    ...
    UWrMaxSATComp(const UWrMaxSATComp&) = delete;
    UWrMaxSATComp& operator=(const UWrMaxSATComp&) = delete;
...
PYBIND11_MODULE(urmaxsat_comp_py, m) {
    m.doc() = "pybind11 plugin for UWrMaxSAT 1.4";

    py::class_<UWrMaxSATComp>(m, "UWrMaxSAT")
        .def(py::init<>())
        .def("newVar", &UWrMaxSATComp::newVar, "Generates a new variable ID.")
        .def("addClause", &UWrMaxSATComp::addClause,
             py::arg("clause"), py::arg("weight") = std::nullopt,
             "Adds a clause. If weight is None, it's a hard clause.")
        .def("assume", &UWrMaxSATComp::assume, "Add assumptions for the next solve call.")
        .def("solve", &UWrMaxSATComp::solve, "Solve the instance under the current assumptions.")
        .def("getCost", &UWrMaxSATComp::getCost, "Get the cost of the optimal solution.")
        .def("getValue", &UWrMaxSATComp::getValue, "Get the truth value of a literal in the model.")
        .def("set_terminate", &UWrMaxSATComp::set_terminate, "Set a termination callback.")
        .def("signature", &UWrMaxSATComp::signature, "Get the solver signature string.");
}
