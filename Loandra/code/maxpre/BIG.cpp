...
void Preprocessor::genIndex(vector<vector<int> >& g, vector<vector<int> >& rg, int x, int u1, int u2, int& stamp, vector<int>& le, vector<int>& ri, vector<int>& up1, vector<int>& up2, int order) {
	...
	if (order == 0) {
		shuffle_with_rand(ne.begin(), ne.begin() + g[x].size());
		shuffle_with_rand(ne.begin() + g[x].size(), ne.end());
	}
	else if (order == 1) {
		shuffle_with_rand(ne.begin(), ne.begin() + rg[x].size());
		shuffle_with_rand(ne.begin() + rg[x].size(), ne.end());
	}
	else if (order == 2) {
		shuffle_with_rand(ne.begin(), ne.end());
	}
	else {
		assert(0);
	}
...
int Preprocessor::tryBIG(int lit, bool doTC) {
	...
	for (int trys = 0; trys < opt.BIG_tries; trys++) {
		shuffle_with_rand(perm.begin(), perm.end());
