...
from hermax.non_incremental.incomplete import OpenWBOInc, SPBMaxSATCFPS
import pytest


def _require_spb():
    if not SPBMaxSATCFPS.is_available():
        import pytest

        pytest.skip("SPB-MaxSAT-c-FPS binary is not available in this environment.")
...
def test_spb_basic_weighted_smoke():
    _require_spb()
    s = SPBMaxSATCFPS()
...
def test_spb_assumptions_emulated_by_hard_units():
    _require_spb()
    s = SPBMaxSATCFPS()
