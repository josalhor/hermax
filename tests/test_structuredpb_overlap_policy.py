...
def test_baseline_paper_matches_first_fit_known_case(pbamo_module) -> None:
    lits = [1, 2, 3, 4, 5, 6]
    weights = [5, 5, 5, 4, 4, 4]
...
def test_dynamic_future_known_case_expected_partition_and_better_amo_cap(pbamo_module) -> None:
    lits = [1, 2, 3, 4, 5, 6]
    weights = [5, 5, 5, 4, 4, 4]
    amo_groups = [[1, 2, 4], [2, 3, 5], [1, 3, 6], [4, 5], [5, 6]]
...
def test_dynamic_future_returns_valid_partition_on_deterministic_overlap_cases(pbamo_module) -> None:
    cases = [
        {
            "lits": [1, 2, 3, 4, 5],
            "weights": [9, 8, 7, 2, 1],
    ...
    for case in cases:
        partition = pbamo_module.choose_overlap_partition(
            case["lits"],
            case["weights"],
            amo_groups=case["amo"],
            eo_groups=case["eo"],
...
def test_dynamic_future_exact_partition_on_secondary_cases(pbamo_module) -> None:
    case_a = pbamo_module.choose_overlap_partition(
        [1, 2, 3, 4, 5],
        [9, 8, 7, 2, 1],
    ...
    case_b = pbamo_module.choose_overlap_partition(
        [1, 2, 3, 4, 5, 6],
        [10, 9, 8, 2, 2, 2],
