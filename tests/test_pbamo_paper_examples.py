...
def test_paper_intro_example_mdd_matches_oracle_and_shrinks_formula(
    pbamo_module, pb_baseline, card_baseline, sat_solver_name: str
) -> None:
    lits = [1, 2, 3, 4, 5, 6]
    weights = [2, 3, 4, 2, 3, 4]
    groups = [[1, 2, 3], [4, 5, 6]]
    ...
    cnf = pbamo_module.PBAMOEnc.leq(
        ...
        bound=bound,
        top_id=max(lits),
        encoding=pbamo_module.EncType.mdd,
        emit_amo=True,
    )
...
def test_paper_intro_example_rggt_matches_oracle_and_is_even_smaller(
    ...
    cnf = pbamo_module.PBAMOEnc.leq(
        ...
        bound=bound,
        top_id=max(lits),
        encoding=pbamo_module.EncType.rggt,
        emit_amo=True,
    )
...
def test_paper_motivating_example_rggt_pb_side_simplifies_to_one_clause(pbamo_module) -> None:
    lits = [1, 2, 3, 4, 5, 6]
    weights = [2, 3, 4, 2, 3, 4]
    groups = [[1, 2, 3], [4, 5, 6]]
    bound = 7
...
def test_paper_rggt_fig5_x7_is_irrelevant_under_fixed_x1_to_x6(pbamo_module) -> None:
    lits = [1, 2, 3, 4, 5, 6, 7]
...
def test_paper_rggt_fig5_matches_removed_x7_instance_size(pbamo_module) -> None:
    with_x7 = pbamo_module.PBAMOEnc.leq(
        lits=[1, 2, 3, 4, 5, 6, 7],
        weights=[20, 30, 20, 40, 10, 20, 1],
        groups=[[1, 2], [3, 4], [5, 6], [7]],
        bound=55,
    ...
    without_x7 = pbamo_module.PBAMOEnc.leq(
        lits=[1, 2, 3, 4, 5, 6],
