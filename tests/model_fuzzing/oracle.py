...
def environments(case: Case) -> Iterator[dict[str, int]]:
    names = [*(f"b{index}" for index in range(case.bool_count)), *(f"i{index}" for index in range(len(case.int_domains)))]
    domains = [*(range(2) for _ in range(case.bool_count)), *(range(lb, ub + 1) for lb, ub in case.int_domains)]
    ...
    for env in environments(case):
        ...
        try:
            ...
            for constraint in case.constraints:
                model &= constraint.emit(bools, ints)
            result = model.solve()
