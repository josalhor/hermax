...
def test_int_instantiation_emits_ladder_domain_clauses_in_expected_shape():
    m = Model()
    speed = m.int("speed", lb=0, ub=4)  # compact ladder => ub-lb=4 threshold lits
    ts = speed._threshold_lits
    assert [t.id for t in ts] == [1, 2, 3, 4]
...
def test_mixed_container_instantiation_allocates_monotonic_noncolliding_ids():
    m = Model()
    bv = m.bool_vector("bv", length=3)
    iv = m.int_vector("iv", length=2, lb=0, ub=5)          # 2 * 5 threshold lits
    ev = m.enum_vector("ev", length=2, choices=["r", "g"])  # 2 * 2 choice lits
    mat = m.int_matrix("mat", rows=2, cols=2, lb=0, ub=3)    # 4 * 3 threshold lits

    ids = []
    ids.extend([lit.id for lit in bv])  # 3
    for cell in iv:
        ids.extend([lit.id for lit in cell._threshold_lits])  # 10
    for enum_cell in ev:
        ids.extend([lit.id for lit in enum_cell._choice_lits.values()])  # 4
    for r in range(2):
        for c in range(2):
            ids.extend([lit.id for lit in mat._grid[r][c]._threshold_lits])  # 12

    assert len(ids) == 3 + 10 + 4 + 12
