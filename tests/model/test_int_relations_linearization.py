...
def test_intvar_equality_linear_clause_shape_and_count_same_domain():
    ...
    rel = x == y
    assert isinstance(rel, ClauseGroup)
    assert len(rel._clauses) == 2 * len(x._threshold_lits)
    assert all(1 <= len(c) <= 2 for c in rel._clauses)
...
def test_intvar_order_relations_linear_clause_shape_and_count_same_domain():
    ...
    assert len(le._clauses) == len(x._threshold_lits)
    assert len(ge._clauses) == len(x._threshold_lits)
    assert all(1 <= len(c) <= 2 for c in le._clauses + ge._clauses)

    # Strict versions remain linear and compact (often cheaper than
    # non-strict + inequality after offset-aware direct compilation).
    assert all(1 <= len(c) <= 2 for c in lt._clauses + gt._clauses)
    assert len(lt._clauses) <= len(le._clauses) + 1
    assert len(gt._clauses) <= len(ge._clauses) + 1
...
def test_intvar_inequality_no_new_vars_linear_and_clause_width_bounded():
    ...
    overlap = min(x.ub, y.ub) - max(x.lb, y.lb) + 1
    assert len(neq._clauses) == overlap
    assert top_after == top_before  # no new vars
    assert all(len(c) <= 4 for c in neq._clauses)


def test_intvar_inequality_singleton_overlap_produces_empty_clause_when_impossible():
    m = Model()
    x = m.int("x", lb=5, ub=5)  # singleton {5}
    y = m.int("y", lb=5, ub=5)  # singleton {5}
    neq = x != y
    assert len(neq._clauses) >= 1
    m &= neq
