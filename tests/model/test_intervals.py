...
def test_interval_construction_creates_start_end_and_enforces_duration_relation():
    ...
    # start in [0, 19], end in [5, 24]
    assert task.start.lb == 0 and task.start.ub == 19
    assert task.end.lb == 5 and task.end.ub == 24
...
def test_interval_identity_is_linear_ladder_weld_without_pb_aux_blowup():
    ...
    ncuts = len(task.start._threshold_lits)

    # Domain constraints:
    # - start ladder: (ncuts-1) monotonic over compact ncuts-bit ladder
    # - end ladder:   (ncuts-1) monotonic over compact ncuts-bit ladder
    # Weld constraints:
    # - 2 binary clauses per threshold bit = 2*ncuts
    expected_added_hard = (ncuts - 1) + (ncuts - 1) + (2 * ncuts)
    ...
    assert len(task.start._threshold_lits) == ncuts
    assert len(task.end._threshold_lits) == ncuts
    assert len(m._registry) == 2 * ncuts

    # Weld clauses should be binary and pairwise over the aligned ladders.
    weld = m._hard[-(2 * ncuts):]
    for cl in weld:
