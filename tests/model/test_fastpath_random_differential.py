...
def _solve_with_path_control(
    ...
    *,
    ...
    orig = getattr(_EncoderDispatch, fastpath_name)
    matched = {"n": 0}
    ...
    try:
        if disable:
            setattr(_EncoderDispatch, fastpath_name, staticmethod(lambda *args, **kwargs: None))
        else:
            setattr(_EncoderDispatch, fastpath_name, staticmethod(wrapped))

        m, vars_int, vars_bool, truth = build_model()
        r = m.solve()
        return r, vars_int, vars_bool, truth, matched["n"]
    finally:
        setattr(_EncoderDispatch, fastpath_name, orig)
...
def test_random_diff_univariate_fastpath():
    def make_case(rng: random.Random):
        ...
        op = rng.choice(["<=", "<", ">=", ">", "=="])
        c = rng.randint(-12, 12)
        pin = rng.choice([None, rng.randint(lb, ub)])
...
def test_random_diff_univariate_bool_fastpath():
    def make_case(rng: random.Random):
        ...
        op = rng.choice(["<=", "<", ">=", ">", "=="])
        c = rng.randint(-10, 12)
        pin_x = rng.choice([None, rng.randint(lb, ub)])
