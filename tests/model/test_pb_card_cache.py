...
def _count_calls(monkeypatch):
    ...
    orig_auto_leq = PBAMOEnc.auto_leq
    orig_auto_eq = getattr(PBAMOEnc, "auto_eq", None)
    orig_atmost = CardEnc.atmost
    orig_atleast = CardEnc.atleast
    orig_ceq = CardEnc.equals
    orig_leq = PBEnc.leq
    orig_geq = PBEnc.geq
    orig_peq = PBEnc.equals

    def auto_leq(*args, **kwargs):
        calls["structured_auto_leq"] += 1
        return orig_auto_leq(*args, **kwargs)

    def auto_eq(*args, **kwargs):
        calls["structured_auto_eq"] += 1
        if orig_auto_eq is None:
            raise NotImplementedError("PBAMOEnc.auto_eq is not implemented")
        return orig_auto_eq(*args, **kwargs)
    ...
    monkeypatch.setattr(PBAMOEnc, "auto_leq", staticmethod(auto_leq))
    monkeypatch.setattr(PBAMOEnc, "auto_eq", staticmethod(auto_eq), raising=False)
    monkeypatch.setattr(CardEnc, "atmost", atmost)
    monkeypatch.setattr(CardEnc, "atleast", atleast)
    monkeypatch.setattr(CardEnc, "equals", ceq)
    monkeypatch.setattr(CardEnc, "atmost", atmost)
    monkeypatch.setattr(CardEnc, "atleast", atleast)
    monkeypatch.setattr(CardEnc, "equals", ceq)
    monkeypatch.setattr(PBEnc, "leq", leq)
    monkeypatch.setattr(PBEnc, "geq", geq)
    monkeypatch.setattr(PBEnc, "equals", peq)
...
def test_pb_cache_geq_reused(monkeypatch):
    ...
    m &= (2 * a + 3 * b >= 3)
    m._commit_pb()
    # Unified pipeline normalizes >= into a <= form over negated literals.
    assert calls["pb_leq"] == 1
