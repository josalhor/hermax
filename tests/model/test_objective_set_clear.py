...
def test_obj_set_tracks_constant_delta_on_replace():
    ...
    false_dim = m._lit_to_dimacs(m._get_bool_constant_literal(False))
    w1 = sum(w for w, c in m._soft if w > 0 and len(c) == 1 and c[0] == false_dim)
    m.obj = a + 2
    c2 = m._objective_constant
    w2 = sum(w for w, c in m._soft if w > 0 and len(c) == 1 and c[0] == false_dim)
    assert c1 == c2 == 0
    assert w1 != w2
    assert w2 == 2
