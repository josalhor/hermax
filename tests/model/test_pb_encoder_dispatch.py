...
def _patch_card_methods(monkeypatch, calls):
    real_atmost = CardEnc.atmost
    real_atleast = CardEnc.atleast
    real_equals = CardEnc.equals
    ...
    monkeypatch.setattr(CardEnc, "atmost", staticmethod(atmost))
    monkeypatch.setattr(CardEnc, "atleast", staticmethod(atleast))
    monkeypatch.setattr(CardEnc, "equals", staticmethod(equals))


def _patch_pb_methods(monkeypatch, calls):
    real_leq = PBEnc.leq
    real_geq = PBEnc.geq
    real_equals = PBEnc.equals
    ...
    monkeypatch.setattr(PBEnc, "leq", staticmethod(leq))
    monkeypatch.setattr(PBEnc, "geq", staticmethod(geq))
    monkeypatch.setattr(PBEnc, "equals", staticmethod(equals))


def _patch_structured_auto(monkeypatch, calls):
    real_multi_leq = pbamo_mod.PBAMOEnc.multi_leq
    real_auto_leq = pbamo_mod.PBAMOEnc.auto_leq
    real_auto_eq = pbamo_mod.PBAMOEnc.auto_eq

    def wrapped_multi_leq(*args, **kwargs):
        lits = list(kwargs.get("lits", args[0] if len(args) > 0 else []))
        stubs = kwargs.get("stubs", args[1] if len(args) > 1 else [])

        # Heuristic to fill legacy fields for test assertions:
        # 1. Find the "main" PB constraint (one with non-unit weights)
        main_stub = next((s for s in stubs if any(w != 1 for w in s.weights)), stubs[0] if stubs else None)
        weights = list(main_stub.weights) if main_stub else []
        bound = int(main_stub.bound) if main_stub else 0

        # 2. Extract structural groups from the stubs in this cluster
        amo_groups = []
        eo_groups = []
        for s in stubs:
            if s.bound == 1 and all(w in (0, 1) for w in s.weights):
                group = [lits[i] for i, w in enumerate(s.weights) if w == 1]
                if s.op == "<=":
                    amo_groups.append(group)
                else:
                    eo_groups.append(group)
        ...
        return real_multi_leq(*args, **kwargs)

    monkeypatch.setattr(
        pbamo_mod.PBAMOEnc,
        "multi_leq",
        classmethod(lambda cls, *a, **kw: wrapped_multi_leq(*a, **kw)),
    )
    ...
    def wrapped_auto_eq(*args, **kwargs):
        calls.append(
            (
                "structured.auto_eq",
                list(kwargs["lits"]),
        ...
        return real_auto_eq(*args, **kwargs)

    monkeypatch.setattr(
        pbamo_mod.PBAMOEnc,
        "auto_eq",
        classmethod(lambda cls, *a, **kw: wrapped_auto_eq(*a, **kw)),
    )
...
def test_commit_pb_routes_zero_amo_branch_through_structured_entry(monkeypatch):
    calls = []
    _patch_card_methods(monkeypatch, calls)
    _patch_pb_methods(monkeypatch, calls)
    _patch_structured_auto(monkeypatch, calls)
    ...
    assert [c0 for c0, *_ in calls] == ["structured.auto_leq", "pb.leq"]
...
def test_commit_pb_does_not_expand_large_int_exact_value_family(monkeypatch):
    ...
    assert [entry[0] for entry in calls] == ["structured.auto_leq", "pb.leq"]
