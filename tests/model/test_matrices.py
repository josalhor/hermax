...
def test_int_matrix_construction_shape_and_element_names():
    ...
    assert all(len(cell._threshold_lits) == 4 for row in mat._grid for cell in row)
...
def test_int_matrix_invalid_bounds_propagate_from_int_cells():
    ...
    with pytest.raises(ValueError):
        m.int_matrix("m", rows=2, cols=2, lb=3, ub=2)


def test_matrix_declaration_emits_cell_domain_constraints_in_export():
    m = Model()
    mat = m.int_matrix("m", rows=2, cols=2, lb=0, ub=3)  # each cell contributes 3 threshold lits + 2 clauses

    # IDs should be contiguous across the 4 cells * 3 threshold lits.
    ids = [lit.id for row in mat._grid for cell in row for lit in cell._threshold_lits]
    assert ids == list(range(1, 13))

    cnf = m.to_cnf()
    # Each cell contributes 2 monotonicity clauses in the compact ladder.
