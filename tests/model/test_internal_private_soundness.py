...
def test_private_bound_from_zero_compare(op, const, expected):
    assert _EncoderDispatch._bound_from_zero_compare(op, const) == expected
...
def test_private_extract_multi_int_affine_accepts_full_lift_and_rejects_partial_or_neg_polarity():
    ...
    ok = _EncoderDispatch._extract_multi_int_affine(m, (2 * x) + (3 * y) + 7)
    ...
    assert _EncoderDispatch._extract_multi_int_affine(m, partial) is None

    # Negative-polarity threshold literal is rejected.
    neg = hm.PBExpr(m, [Term(1, ~x.__ge__(3))], 0)
    assert _EncoderDispatch._extract_multi_int_affine(m, neg) is None
...
def test_private_int_cmp_constraint_matches_semantics(op, k):
    m = Model()
    x = m.int("x", 1, 8)
    out = _EncoderDispatch._int_cmp_constraint(x, op, k)
...
def test_private_lit_implies_constant_folding_semantics():
    ...
    _EncoderDispatch._lit_implies(clauses, m, True, b)   # b
    _EncoderDispatch._lit_implies(clauses, m, False, b)  # tautology
    _EncoderDispatch._lit_implies(clauses, m, a, True)   # tautology
    _EncoderDispatch._lit_implies(clauses, m, a, False)  # ~a
    _EncoderDispatch._lit_implies(clauses, m, a, b)      # ~a v b
...
def test_private_equiv_literals_group_constant_folding_semantics():
    ...
    assert len(g_same._clauses) == 0

    # true <-> false is contradiction
    g_tf = m._equiv_literals_group(t, f)
    assert len(g_tf._clauses) == 1 and len(g_tf._clauses[0]) == 0

    # true <-> a  =>  (a)
    g_ta = m._equiv_literals_group(t, a)
    assert len(g_ta._clauses) == 1
    assert g_ta._clauses[0] == (a.id,)

    # false <-> a => (~a)
    g_fa = m._equiv_literals_group(f, a)
    assert len(g_fa._clauses) == 1
    assert g_fa._clauses[0] == (-a.id,)


def test_private_collapse_terms_int_fastpath_preserves_first_seen_order_and_cancels_zeros():
    m = Model()
    a = m.bool("a")
    b = m.bool("b")
    terms = [
    ...
    out = hm.PBExpr._collapse_terms(terms)
    assert [(t.literal, int(t.coefficient)) for t in out] == [(b, 1), (a, 4)]


def test_private_collapse_terms_no_duplicates_returns_terms_unchanged():
    m = Model()
    a = m.bool("a")
    b = m.bool("b")
    terms = [Term(2, a), Term(-3, b)]
    out = hm.PBExpr._collapse_terms(terms)
    assert out is terms
    assert out[0] is terms[0]
    assert out[1] is terms[1]


def test_private_collapse_terms_float_path_drops_near_zero_coefficients():
    m = Model()
    a = m.bool("a")
    terms = [
        Term(1.0, a),
...
def test_private_pbexpr_from_item_term_does_not_call_collapse_terms(monkeypatch):
    m = Model()
    a = m.bool("a")
    term = Term(3, a)

    def _boom(_terms):
        raise AssertionError("collapse_terms should not run for trusted single-term from_item")

    monkeypatch.setattr(hm.PBExpr, "_collapse_terms", staticmethod(_boom))
    expr = hm.PBExpr.from_item(term)
    assert expr.terms == [term]
    assert expr.constant == 0


def test_private_pbexpr_merge_disjoint_skips_collapse_terms(monkeypatch):
    m = Model()
    a = m.bool("a")
    b = m.bool("b")

    lhs = hm.PBExpr.from_item(a)
    rhs = hm.PBExpr.from_item(b)

    def _boom(_terms):
        raise AssertionError("collapse_terms should not run for disjoint trusted merge")

    monkeypatch.setattr(hm.PBExpr, "_collapse_terms", staticmethod(_boom))
    out = lhs + rhs
    assert [(t.literal, int(t.coefficient)) for t in out.terms] == [(a, 1), (b, 1)]


def test_private_pbexpr_merge_overlapping_still_uses_collapse_terms():
    m = Model()
    a = m.bool("a")
    lhs = hm.PBExpr.from_item(a)
    rhs = hm.PBExpr.from_item(a)
    out = lhs + rhs
    assert len(out.terms) == 1
    assert out.terms[0].literal is a
    assert int(out.terms[0].coefficient) == 2
