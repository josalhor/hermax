...
def _build_pbamo() -> None:
    try:
        from hermax.internal import _pbamo  # type: ignore
...
def pbamo_module():
    ...
    sys.modules.pop("hermax.encoder.pbamo", None)
    sys.modules.pop("hermax.internal._pbamo", None)
    return importlib.import_module("hermax.encoder.pbamo")
...
def assert_encoding_matches_oracle_and_baseline(
    *,
    ...
    enc_type = getattr(pbamo_module.EncType, encoding_name)
