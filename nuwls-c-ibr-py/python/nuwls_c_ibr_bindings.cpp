...
class NuWLSCIBRWrapper {
public:
    NuWLSCIBRWrapper() : n_vars_ext_(0) {}
...
PYBIND11_MODULE(nuwls_c_ibr, m) {
    m.doc() = "pybind11 bindings for NuWLS-c-IBR (NuWLS-c / BLS path)";

    py::class_<NuWLSCIBRWrapper>(m, "NuWLSCIBR")
        .def(py::init<>())
        .def("newVar", &NuWLSCIBRWrapper::newVar)
        .def("setNInputVars", &NuWLSCIBRWrapper::setNInputVars)
        .def("addClause", &NuWLSCIBRWrapper::addClause, py::arg("clause"), py::arg("weight") = py::none())
        .def("solve", &NuWLSCIBRWrapper::solve, py::arg("assumptions") = py::none())
        .def("getCost", &NuWLSCIBRWrapper::getCost)
        .def("getValue", &NuWLSCIBRWrapper::getValue)
        .def("getModel", &NuWLSCIBRWrapper::getModel);
}
