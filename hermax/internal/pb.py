...
# checking whether or not internal pblib bindings are available
pblib_present = True
pblib = None
try:
    from hermax.internal import _pblib as pblib
except ImportError:
    pblib_present = False
...
class PBEnc(object):
    ...
    def _encode(cls, lits, weights=None, bound=1, top_id=None, vpool=None,
        ...
        assert pblib_present, 'Internal package \'hermax.internal._pblib\' is unavailable. Check your installation.'

        ret.clauses, max_aux = pblib.encode_pb(
            wlits=wlits,
            comparator=EncType._to_pbcmp[comparator],
            bound=bound,
            top_id=top_id,
            pb_encoder=EncType._to_pbenc[encoding],
            conditionals=conditionals,
        )
        ret.nv = max(ret.nv, top_id, max_aux)  # needed if no auxiliary variable is used
