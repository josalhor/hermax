...
from hermax.internal.card import ITotalizer
...
class RC2(object):
    ...
    def compute(self):
        ...
        if res:
            ...
            self.model = sorted(
                [
                    int(copysign(self.vmap.i2e[abs(l)], l))
                    for l in self.model
                    if abs(l) in self.vmap.i2e
                ],
                key=lambda l: abs(l)
            )
    ...
    def adapt_am1(self):
        ...
        while lits:
            ...
            for r in am1_set:
                # Remove only incident edges for removed literals.
                # Equivalent to global pruning while touching fewer adjacency sets.
                nbrs = conns.get(r)
                if not nbrs:
                    continue
                for n in tuple(nbrs):
    ...
    def filter_assumps(self):
        ...
        self.sels_set.difference_update(self.garbage)
...
class RC2Stratified(RC2, object):
    ...
    def next_level(self):
        """
        Behavior-identical optimized version of next_level_baseline.

        Complexity:
        - baseline: O(n^2) due to repeated suffix sums in loop.
        - optimized: O(n) using precomputed suffix counts/sums and
            incremental cluster aggregates.
        """
        ...
        # Precompute selector multiplicities per weight (len(self.wstr[w])).
        cnt_by_weight: dict[int, int] = {}
        for w in self.blop:
            if w not in cnt_by_weight:
                cnt_by_weight[w] = len(self.wstr[w])

        # Suffix aggregates equivalent to:
        # numr = sum(len(wstr[w]) for w in blop[(levl+1):])
        # sumr = sum(w * len(wstr[w]) for w in blop[(levl+1):])
        suffix_numr = [0] * n
        suffix_sumr = [0] * n
        run_numr = 0
        run_sumr = 0
        for i in range(n - 2, -1, -1):
            w_next = self.blop[i + 1]
            c_next = cnt_by_weight[w_next]
            run_numr += c_next
            run_sumr += w_next * c_next
            suffix_numr[i] = run_numr
            suffix_sumr[i] = run_sumr

        # Cluster behavior must be identical to baseline
        cluster = [self.levl]
        first_w = self.blop[self.levl]
        first_c = cnt_by_weight[first_w]
        numc = first_c
        sumc = first_w * first_c

        while self.levl < n - 1:
            wght = self.blop[self.levl]
            numr = suffix_numr[self.levl]
            sumr = suffix_sumr[self.levl]

            if wght > sumr and sumr != 0:
                break

            if div_str and numr / float(n - self.levl - 1) > self.sdiv:
                break
