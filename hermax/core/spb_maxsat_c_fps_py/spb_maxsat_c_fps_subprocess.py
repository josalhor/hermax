...
_WORKER_SOLVER_CLASS = "hermax.core.spb_maxsat_c_fps_py.spb_maxsat_c_fps_solver.SPBMaxSATCFPSSolver"


class SPBMaxSATCFPS(IPAMIRSolver):
    """
    SPB-MaxSAT-c-FPS fake-incremental wrapper with subprocess isolation.

    The wrapper caches the formula in Python and, on each `solve()`, launches a
    one-shot Python worker subprocess that imports the native SPB binding wrapper,
    rebuilds the solver state, solves once, returns the result, and exits.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._last_signature: str = "SPB-MaxSAT-c-FPS (subprocess wrapper)"
    ...
    def _snapshot(self) -> Dict[str, object]:
        return {
            ...
            "hard_clauses": [list(cl) for cl in self._hard_clauses],
    ...
    def solve(
        ...
        if run.timed_out and run.response is None:
            ...
            if raise_on_abnormal:
                raise RuntimeError("SPB worker timed out")
            return False
        ...
        # IPAMIR-facing cost semantics must match the exposed model and wrapper soft map.
        # Backends may report a different internal objective convention.
    ...
    def _apply_fallback_solver_output(self, exit_code: Optional[int], text: str) -> None:
        ...
        self._status = st
        self._last_signature = "SPB-MaxSAT-c-FPS (subprocess worker/native binding)"
