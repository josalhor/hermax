...
_WORKER_SOLVER_CLASS = "hermax.core.tt_openwbo_inc_py.tt_openwbo_inc_solver.TTOpenWBOIncSolver"


class TTOpenWBOInc(IPAMIRSolver):
    @classmethod
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._last_signature: str = "TT-Open-WBO-Inc (subprocess wrapper)"
    ...
    def solve(
        ...
        if run.timed_out and run.response is None:
            ...
            if raise_on_abnormal:
                raise RuntimeError("TT-Open-WBO-Inc worker timed out")
            return False
    ...
    def _apply_fallback_solver_output(self, exit_code: Optional[int], text: str) -> None:
        ...
        # TT's Topor-backed algorithms sometimes terminate the worker with only
        # `s SATISFIABLE` on stdout even when they also print a final objective
        # and model. In this wrapper those runs are treated as completed solves.
        if st == SolveStatus.INTERRUPTED_SAT and obj is not None and model is not None:
        ...
        self._status = st
        self._last_signature = "TT-Open-WBO-Inc (subprocess worker/native binding)"
