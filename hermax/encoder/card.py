...
import hermax_pycard as pycard
import signal
...
class ITotalizer(object):
    ...
    def new(self, lits=[], ubound=1, top_id=None):
        ...
        self.ubound = ubound
        self.top_id = max(self._max_abs_lit, top_id if top_id != None else 0)
    ...
    def extend(self, lits=[], ubound=None, top_id=None):
        ...
        self._max_abs_lit = max(self._max_abs_lit, max_new_abs)
        self.top_id = max(self._max_abs_lit, self.top_id, top_id if top_id != None else 0)
    ...
    def merge_with(self, another, ubound=None, top_id=None):
        ...
        self._lits_set.update(another.lits)
        self._max_abs_lit = max(self._max_abs_lit, another._max_abs_lit)
